...
import { assertScope, hasScope } from './device-api.mjs';
...
async function requireImporter(request, env) {
  ...
  if (!hasScope(device, 'asset.upload') && !hasScope(device, 'catalog.publish')) {
    throw new ApiError(403, 'importer_required', 'A TV credential cannot publish library files.');
  }
  return device;
}

export async function routeImporterApi(request, env) {
  const device = await requireImporter(request, env);
  const url = new URL(request.url);
  const allowed = [
    '/api/importer/status',
    '/api/importer/uploads',
    '/api/importer/reserve-upload',
    '/api/importer/finalize-upload',
    '/api/importer/publish'
  ];
  const isAllowed = allowed.some((prefix) => url.pathname.startsWith(prefix));
  if (!isAllowed) {
    throw new ApiError(403, 'importer_forbidden_route', 'Importers can only access upload and publication endpoints.');
  }
