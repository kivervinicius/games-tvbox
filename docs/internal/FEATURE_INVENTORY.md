...
| Importação privada do Fire Stick | copia ROMs, saves, catálogo, capas, slides e playlists para cache privado | preservar acervo já instalado | operação | Disponível com confirmação | `Import-FireStickLibrary`, `Sync-FireStickToPrivateRepo` | botão Importar e publicar privado | ADB, checkout Git privado e DPAPI | Pages não recebe ROMs; ADB e Git exigem ação do usuário | `Test-DeviceImport.ps1`, `Test-GitHubCatalog.ps1` | `operations/private-library.md` |
| Catálogo dinâmico offline | atualiza jogos, capas e metadados sem novo APK | manter catálogo atual | integração | Disponível | `Sync-CatalogToFireStick`, `CatalogStore` | Manager | ADB; fallback embutido | recompilação necessária só para código | `Test-CatalogSync.ps1`, `catalog-store.tests.ps1` | `quick-start.md` |
