...
A ação importa a biblioteca para a área privada, lê o inventário do Fire Stick, mescla o catálogo importado, aplica a sanitização do catálogo privado e publica `catalog.private.json` junto das cópias allowlisted de `roms`, `saves`, `covers`, `theme` e `playlists` no checkout privado. Ela não chama `adb shell rm`, `mv` ou comandos de exclusão; não remove nem substitui ROMs, saves, favoritos ou configurações no Fire Stick. Arquivos já existentes no checkout privado não são sobrescritos; diferenças ficam registradas em `private-sync-report.json` na importação local.
...
Para uso independente em vários aparelhos, o Manager também pode gerar `library.manifest.json` com tamanho, SHA-256, caminho local no Fire Stick e URL de asset de um GitHub Release privado. Cada launcher consulta esse manifesto diretamente, baixa os itens faltantes em sequência e mantém a cópia local para uso offline. O download usa arquivo temporário e validação antes de substituir qualquer destino; o progresso aparece na galeria sem bloquear os jogos já instalados.

## Limites e ações manuais

O GitHub Pages recebe apenas a galeria pública selecionada e itens legais para redistribuição, como homebrew, demos e domínio público. ROMs comerciais, saves, caminhos pessoais, tokens e links privados nunca são publicados no site público; os arquivos pessoais ficam somente no checkout privado verificado. Pages é um site estático: ele não armazena sua biblioteca privada, não autentica o Fire Stick e não transfere ROMs.
